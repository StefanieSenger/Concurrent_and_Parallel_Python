# task: https://www.linkedin.com/learning/python-parallel-and-concurrent-programming-part-2/challenge-merge-sort-in-python?resume=false&u=72605090
...
def par_mergesort(array, pool=None, *args):
    if not pool:
        with ProcessPoolExecutor() as executor:
            futures = par_mergesort(array, pool=executor, *args)
            return futures
    else:
        if not args: # first call
            seq_mergesort(array, 0, len(array)-1)
...
if __name__ == '__main__':
    ...
    #array = [random.randint(0,10) for i in range(4)]
    #array = [9,8,1]
