...
serving_line = mp.Queue(5) # queue class specifically designed for communication between processes

def cpu_work(work_units):
    """simulates heavy cpu work
    with this heavy work, the queue would quickly run full, because the PIL only
    allows one thread to work at the same time"""
    x = 0
